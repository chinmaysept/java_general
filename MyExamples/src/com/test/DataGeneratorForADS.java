...
public class DataGeneratorForADS {
	...
	DataGeneratorForADS(BigInteger STORE_NO, BigInteger PRIMARY_ACCT_NO,
			BigInteger PURCHASE_ORDER, BigInteger INVOICE_NO,
			BigInteger Stan_no, String Filename) throws Exception {
	...
	public void Write_to_File() {
		try {
			for (int i = 0; i < 3; i++) {
				sbf2.append(sbf2.append(DIV_NO + STORE_NO + ACCT_NO
						+ PRIMARY_ACCT_NO + TRAN_AMT + SWIPE_IND + SERIES
						+ EXPIRE_MM + EXPIRE_CCYY + AUTH_TYPE + SOURCE_CODE
						+ DECISION_to_Authcode + NET_AMT + DOWN_PMT_to_Bal
						+ AVS_5DIGIT_ZIP_to_address + MATCH_NUMBER
						+ PURCHASE_ORDER + INVOICE_NO + TRAN_DATE_time
						+ CVC2_CD_to_data_type + Stan_no
						+ unsettled_amt_to_filler));
				sbf2.append("\n");
			}
	...
	public static void main(String[] args) throws Exception {
		...
		new DataGeneratorForADS(new BigInteger("00001"), new BigInteger(
				"000000000000000001"), new BigInteger("22222222222222222222"),
				new BigInteger("00000000001000000000"),
				new BigInteger("100000"), "ADS_INPUT_FILE_11242011");
